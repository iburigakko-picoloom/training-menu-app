...
function formatSeconds(sec){const total=Math.max(0,Math.round(Number(sec)||0)),minutes=Math.floor(total/60),seconds=total%60;return seconds?`${minutes}分 ${seconds}秒`:`${minutes}分`}
...
function renderCreateScreen(){
  ...
  menus.forEach(menu=>{
    ...
    row.innerHTML=`<span class="select-mark ${selected?'selected':''}">${selected?order:''}</span><span><span class="row-name">${escapeHTML(menu.name)}</span><span class="row-meta">${escapeHTML(categoryName(menu.categoryId))} / ${formatCompactSeconds(menu.seconds)}${menu.requiresSets?' / セットあり':''}</span></span>`;
...
function renderSetScreen(){
  ...
  setPlan.forEach((row,i)=>{const menu=findMenu(row.menuId);if(!menu)return;const tr=document.createElement('tr');if(menu.requiresSets){tr.innerHTML=`<td><div class="set-name">${i+1}. ${escapeHTML(menu.name)}<small>${formatCompactSeconds(menu.seconds)} / 1set・1人</small></div></td>`+selectedPeople.map(p=>`<td>${stepperHTML(i,p,getSets(row,p))}</td>`).join('')}else{tr.innerHTML=`<td><div class="set-name">${i+1}. ${escapeHTML(menu.name)}<small>${formatCompactSeconds(menu.seconds)}</small></div></td>`+(selectedPeople.length?`<td class="once-time-cell" colspan="${selectedPeople.length}"><span class="once-pill">${formatCompactSeconds(menu.seconds)}</span></td>`:'')}tbody.appendChild(tr)});
...
function renderListScreen(){
  ...
  menus.forEach(menu=>{
    ...
    const mode=menu.requiresSets?`${formatCompactSeconds(menu.seconds)} / セットあり`:`${formatCompactSeconds(menu.seconds)}`;
