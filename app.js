...
function renderSetScreen(){
  ...
  setPlan.forEach((row,i)=>{const menu=findMenu(row.menuId);if(!menu)return;const tr=document.createElement('tr');if(menu.requiresSets){tr.innerHTML=`<td><div class="set-name">${i+1}. ${escapeHTML(menu.name)}<small>${formatCompactSeconds(menu.seconds)} / 1set・1人</small></div></td>`+selectedPeople.map(p=>`<td>${stepperHTML(i,p,getSets(row,p))}</td>`).join('')}else{tr.innerHTML=`<td><div class="set-name">${i+1}. ${escapeHTML(menu.name)}<small>${formatCompactSeconds(menu.seconds)}</small></div></td>`+(selectedPeople.length?`<td colspan="${selectedPeople.length}"><span class="once-pill">${formatCompactSeconds(menu.seconds)}</span></td>`:'')}tbody.appendChild(tr)});
